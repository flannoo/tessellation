...
trait ConsensusStateUpdater[F[_], Key, Artifact, Context] {
  ...
  /** Tries to conditionally update a consensus based on information collected in `resources`, this includes:
    ...
    */
  def tryUpdateConsensus(key: Key, resources: ConsensusResources[Artifact, Context]): F[StateUpdateResult]
  ...
  def trySpreadAck(
    ...
    resources: ConsensusResources[Artifact, Context]
  ): F[StateUpdateResult]
...
object ConsensusStateUpdater {
  ...
  def make[F[
    ...
    private def updateConsensus(resources: ConsensusResources[Artifact, Context])(
    ...
    private def advanceStatus(
      ...
      StateT[F, ConsensusState[Key, Artifact, Context], F[Unit]] { state =>
        if (state.locked)
          ...
          state.status match {
            case CollectingFacilities(_, lastSignedArtifact, lastContext, ownFacilitatorsHash) =>
                      ...
                      state.facilitators.hashF.flatMap { facilitatorsHash =>
                        for {
                          ...
                          (artifact, returnedEvents) <- consensusFns
                            .createProposalArtifact(state.key, lastSignedArtifact, lastContext, majorityTrigger, events)
            ...
            case CollectingProposals(majorityTrigger, proposalInfo, lastSignedArtifact, lastContext, candidates, ownFacilitatorsHash) =>
              ...
              maybeAllProposals.traverseTap(warnIfForking(ownFacilitatorsHash)).flatMap {
                _.map(_.map(_.hash)).flatTraverse { allProposals =>
                  pickValidatedMajority(lastSignedArtifact, lastContext, majorityTrigger, resources)(allProposals).flatMap {
                    maybeMajorityHash =>
                      state.facilitators.hashF.flatMap { facilitatorsHash =>
                        maybeMajorityHash.traverse { majorityHash =>
                          val newState =
                            state.copy(status =
                              CollectingSignatures[Artifact, Context](majorityHash, majorityTrigger, candidates, facilitatorsHash)
                            )
                ...
                _.map(_.map { case (id, signature) => SignatureProof(PeerId._Id.get(id), signature.signature) }.toList).traverse {
                  ...
                  state.facilitators.hashF.map { facilitatorsHash =>
                    for {
                      ...
                      majorityContext <- resources.contexts.get(majorityHash)
                      signedArtifact = Signed(majorityArtifact, validSignaturesNel)
                      newState = state.copy(status =
                        Finished[Artifact, Context](signedArtifact, majorityContext, majorityTrigger, candidates, facilitatorsHash)
                      )
    ...
    private def getAck(ackKind: PeerDeclarationKind, resources: ConsensusResources[Artifact, Context]): Set[PeerId] = {
